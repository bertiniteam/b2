...
| [0025](0025-randomization-block.md) | Randomization of overdetermined systems is a first-class block _(amended: randomize down to any codimension, #315)_ | Core / system |
...
| [0052](0052-butcher-tables-inline-in-header.md) | ExplicitRKPredictor Butcher tables are C++17 inline members in the header (not out-of-line data), so Windows drops the /WHOLEARCHIVE + /FORCE:MULTIPLE hack (#287); the .cpp is kept only for template instantiations (ADR-0014); don't move the tables back to a .cpp | CI / core |
| [0053](0053-wheels-build-concurrently-with-cpp-tests.md) | Wheel builds run concurrently with the C++ tests (dropped the false `needs: test_cpp_*`); independent compiles, only the cheap doc_lint gate remains; workflow-level result still blocks publish; don't re-add the gate | CI |
| [0054](0054-mp-linear-algebra-via-eigenpy-visitors.md) | Multiprecision linear algebra (bertini.linalg LU/QR/SVD) by instantiating eigenpy's own decomposition visitor templates on the mp matrix types inside _pybertini — reuse, not re-export; one dtype-agnostic surface (mp → eigenpy visitors, double → numpy adapters); bertini.precision is a functional (copy) setter | Python bindings / numpy |
| [0055](0055-mp-complex-to-real-numpy-cast-discards-imag.md) | The mp complex → real numpy cast discards the imaginary part (mirrors numpy's builtin complex128 → float64) instead of `static_cast`-ing through boost, which threw "Could not convert imaginary number to scalar." out of numpy's C cast loop → SIGABRT; don't restore the `static_cast<To>(from)` form | Python bindings / numpy |
| [0056](0056-metadata-for-matches-representatives-final-tolerance.md) | metadata_for matches against multiplicity REPRESENTATIVES at final_tolerance (not the complete copy set at the looser same-point tolerance), so a solver result fed straight back resolves to its representative and is never spuriously "ambiguous"; representatives are ≥ same_point_tolerance apart so the tight window holds at most one; don't restore the old default | Core / nag_algorithms |
| [0057](0057-precision-follows-the-point-not-the-system.md) | Precision follows the POINT: a System carries NO precision at all (no setter, no getter, no member, nothing to prepare or fan out) -- every evaluable type (4 blocks, patch, SLP) self-aligns to the point it is handed under one uniform SyncPrecision, and coefficient generation is a construct-time concern; evaluation aligns instead of refusing, which used to wedge a System with no escape; archive format changed (precision_ no longer serialized); don't re-add a setter, a refusal, or a "prepare" entry point | Core / precision |
| [0058](0058-default-endgame-powerseries.md) | The default endgame is POWER SERIES, matching Bertini 1's documented default (EndgameNum 1), at every choice point (config default, classic parser fallback, CLI, Python factories and solve); measured: Cauchy 190 s vs power series 2.5 s on a regeneration workload, from slow divergers that only Cauchy mishandles; Cauchy stays available by explicit request; don't flip the default back for a workload where Cauchy wins -- pass endgame='cauchy' there | Core / endgames |
| [0059](0059-identity-is-the-encoding-printing-is-a-dialect-family.md) | Identity is the canonical ENCODING, never printed text: the canonicalization tie-break and like-factor key are the node's canonical encoding (lazy, memoized; linear in the DAG -- the second half of #417), b2sysenc bumped to /2; printing is a FAMILY of dialects on the stream -- Classic (`^`, `(re+im*I)`, the Bertini 1 spelling), Python readable (`str`, `**`), Python exact (`repr`, `real_mp('d', p)` / `Complex('re','im', p)` / `Rational('p/q')`, eval rebuilds at full precision); the `(re,im)` form and both Python shims are gone; don't put printed text back into anything a digest depends on, and add a dialect for another system rather than a regex | Core / function_tree |
| [0060](0060-stopping-a-solve-and-reusing-what-it-left.md) | Stopping a solve and reusing what it left: ONE cooperative stop (process-wide request + per-tracker wall-clock DEADLINE) checked between steps, never a watcher thread or an exception across a worker (overrun = one step); every path stamped with where it got to (time, `latest_path_point`, step tally, precision -- no observer, the tracker already knows), abandoned paths RECORDED, never-started ones not; wall-clock budgets OUT of the ask; reuse rule "a record is reusable when the run asks no more of that path than the run that produced it", applied through the user's `RecallPolicy` in its own `RecordsConfig` (Nothing / Completed / Everything); whole-solve budget reads as an interrupt; failed endgame's solution slot is EMPTY (was the previous path's approximation); append new SuccessCodes at the END | Core / trackers, nag_algorithms, records |
| [0061](0061-one-encoding-version-per-release.md) | At most ONE encoding version per RELEASED library version: a registry line is edited IN PLACE while its release is unreleased (new hash, same token) and is append-only once it has shipped, so the number counts encodings that were really written rather than development events (`b2cfgenc/1` and `/2` never shipped at all); every line records the release it first shipped in, no two may claim the same one, and a `.devN` suffix buys no version; the `b2cfgenc` registry gains a COMPOSITION column, because the keyspace hash cannot see which configs the settings text folds in (`/3` and `/4` share a keyspace -- `/4` WAS the composition changing) | Core / records, identity, tests |
