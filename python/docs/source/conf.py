...
# --- autosummary: the auto tier of the API reference ---------------------------------------------
# The exhaustive reference is generated by walking the `bertini` package (see detailed/api.rst and
# the recursive _templates/autosummary/module.rst).  autosummary_generate=True regenerates the stub
# pages under detailed/_autosummary/ on every build, so a namespace change (rename/move a module or
# member) flows through with no hand-editing here.  The curated top-level "Public API" tier is
# authored by hand; this covers everything else.
autosummary_generate = True
autosummary_imported_members = False   # document names where they are DEFINED, not where re-exported
                                       # (avoids duplicating the top-level re-exports onto every page)
