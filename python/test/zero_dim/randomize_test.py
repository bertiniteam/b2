...
total-degree start system needs a square system.  The System.randomize method
...
def test_unequal_degree_single_group_solves_and_filters():
    ...
    zd = pb.ZeroDimSolver(randomized, endgame='cauchy', mptype='adaptive', startsystem='binomial')
    zd.solve()
...
def test_multihomogeneous_bilinear_solves_and_filters():
    ...
    zd = pb.ZeroDimSolver(randomized, endgame='cauchy', mptype='adaptive', startsystem='mhom')
    zd.solve()
    sols = zd.all_solutions()
    assert len(sols) >= 1
...
def test_user_supplied_matrix_combination():
    ...
    randomized = original.randomize([[1, 0, 0], [3, 0, 5]])
    assert randomized.num_functions() == 2

    # at (x, y) = (2, 3): f0 = 12, f2 = 7 -> g0 = 12, g1 = 3*12 + 5*7 = 71.
    g = randomized.eval(np.array([pb.multiprec.complex_mp('2'), pb.multiprec.complex_mp('3')]))
...
def test_randomization_matrix_round_trips():
    ...
    R = original.randomize([[1, 0, 0], [3, 0, 5]]).randomization_matrix()
    assert R.shape == (2, 3)
