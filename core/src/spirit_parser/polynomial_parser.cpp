...
// modified dan brake, april 2015.  notre dame.
...
namespace client
{
	...
	public:
		...
		void addLeaf(Node* leaf){
			leaves.push_back(std::move(std::unique_ptr<Node>(leaf)));
		}
#endif
	...
	struct polynomial : qi::grammar<Iterator,Node*(),ascii::space_type>
	{
		...
		polynomial() : polynomial::base_type(sumexpr) // default constructor for the polynomial class
		{
			namespace phx = boost::phoenix;
			using qi::_1;
			using qi::_val;
			...
			eps[phx::bind(&Node::addLeaf,_val,new Node("null"))] >>  // std::unique_ptr<Node>("null")
