...
unsigned SumOperator::EliminateZeros()
{
	...
	{	
		...
		{
			new_children.push_back(Factory::Make<Integer>(0));
...
std::shared_ptr<Node> SumOperator::Differentiate(std::shared_ptr<Variable> const& v) const
{
	...
	for (int ii = 0; ii < operands_.size(); ++ii)
	{
		...
		counter++;
		if (counter==1)
			ret_sum = Factory::Make<SumOperator>(temp_node,signs_[ii]);
		else
...
void SumOperator::Homogenize(VariableGroup const& vars, std::shared_ptr<Variable> const& homvar)
{
	...
	for (auto iter = operands_.begin(); iter!=operands_.end(); iter++)
	{
		...
		if ( degree_deficiency > 0)
		{
			...
			{
				std::shared_ptr<Node> M = Factory::Make<MultOperator>(homvar,std::dynamic_pointer_cast<Node>(*iter));
				swap(*iter,M);
			}
			else{
				std::shared_ptr<Node> P = Factory::Make<IntegerPowerOperator>(std::dynamic_pointer_cast<Node>(homvar),degree_deficiency);
				std::shared_ptr<Node> M = Factory::Make<MultOperator>(P,std::dynamic_pointer_cast<Node>(*iter));
				swap(*iter,M);
			}
...
{
	return Factory::Make<NegateOperator>(operand_->Differentiate(v));
}
...
unsigned MultOperator::EliminateZeros()
{
	...
	if (have_a_zero) // if there is a single zero, the whole thing should collapse.
	{
		...
		AddOperand(Factory::Make<Integer>(0), true);
		return num_eliminated;
	}
...
std::shared_ptr<Node> MultOperator::Differentiate(std::shared_ptr<Variable> const& v) const
{
	...
	for (int ii = 0; ii < operands_.size(); ++ii)
	{
		...
		auto term_ii = Factory::Make<MultOperator>(local_derivative);
		...
		term_counter++;
		if (term_counter==1)
			ret_sum = Factory::Make<SumOperator>(term_ii,mult_or_div_[ii]);
		else
...
std::shared_ptr<Node> PowerOperator::Differentiate(std::shared_ptr<Variable> const& v) const
{
	...
	auto ret_mult = Factory::Make<MultOperator>(base_->Differentiate(v));
	ret_mult->AddOperand(exponent_);
	ret_mult->AddOperand(Factory::Make<PowerOperator>(base_, exp_minus_one));
	return ret_mult;
}
...
{
	if (exponent_==0)
		return Factory::Make<Integer>(0);
	else if (exponent_==1)
		return operand_->Differentiate(v);
	else if (exponent_==2){
		auto M = Factory::Make<MultOperator>(Factory::Make<Integer>(2), operand_);
		M->AddOperand(operand_->Differentiate(v));
		return M;
	}
	else{
		auto M = Factory::Make<MultOperator>(Factory::Make<Integer>(exponent_),
												Factory::Make<IntegerPowerOperator>(operand_, exponent_-1) );
		M->AddOperand(operand_->Differentiate(v));
		return M;
	}
}
...
{
	auto ret_mult = Factory::Make<MultOperator>(Factory::Make<PowerOperator>(operand_, Factory::Make<Rational>(mpq_rational(-1,2),0)));
	ret_mult->AddOperand(operand_->Differentiate(v));
	ret_mult->AddOperand(Factory::Make<Rational>(mpq_rational(1,2),0));
	return ret_mult;
}
...
{
	return Factory::Make<MultOperator>(operand_,false,operand_->Differentiate(v),true);
}
