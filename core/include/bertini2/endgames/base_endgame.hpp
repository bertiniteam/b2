...
protected:
	...
	using NeededTypes = detail::TypeList<BCT>;  ///< The config types this endgame reads.
	using TupOfVec = typename NeededTypes::ToTupleOfVec;  ///< A tuple of vector containers, one per precision.
	using TupOfReal = typename NeededTypes::ToTupleOfReal;  ///< A tuple of real values, one per precision.
	using TupleOfTimes = typename NeededTypes::template ToTupleOfCont<TimeCont>;  ///< A tuple of time containers, one per precision.
	using TupleOfSamps = typename NeededTypes::template ToTupleOfCont<SampCont>;  ///< A tuple of sample containers, one per precision.
