...
// prototype's cross-impl test (prototypes/ledger_v0/test_cross_impl.py) reads it.
BOOST_AUTO_TEST_CASE(write_the_cross_impl_example)
{
	fs::path const dir = "cross_impl_cpp_output";
	fs::remove_all(dir);
	OutputDirectory out(dir);
	auto const target_id = out.PutDefinition("function f;\nvariable_group x;\nf = x^2-2;\n");
	out.Append({{"kind", "run"}, {"schema", "ledgerrec/1"}, {"when", "2026-07-03 10:00"},
	            {"run", "cppdemo0000000001"}, {"op", "solve"}, {"target_object", target_id},
	            {"num_paths", 2},
	            {"start_points", json::array{
	                json::array{json::array{"1", "0"}},
	                json::array{json::array{"-1", "0"}}}}});
	out.Append({{"kind", "track"}, {"run", "cppdemo0000000001"}, {"index", 0},
	            {"status", "success"},
	            {"endpoint", json::array{json::array{"1.4142135623730951", "0"}}},
	            {"start", json::object{{"kind", "start_label"}, {"index", 0}}}});
	out.Append({{"kind", "track"}, {"run", "cppdemo0000000001"}, {"index", 1},
	            {"status", "success"},
	            {"endpoint", json::array{json::array{"-1.4142135623730951", "0"}}},
	            {"start", json::object{{"kind", "start_label"}, {"index", 1}}}});
	out.RefreshResults();
	BOOST_CHECK(fs::exists(dir / "README.txt"));
	BOOST_CHECK_EQUAL(out.Scan().size(), 3u);
}

// Reads a directory the PYTHON prototype wrote (created by its cross-impl test); skips
// when absent (e.g. wheel CI, where the prototype does not run first).
BOOST_AUTO_TEST_CASE(read_the_python_written_example_if_present)
{
	fs::path const dir = "cross_impl_python_output";
	if (!fs::exists(dir / "history"))
	{
		BOOST_TEST_MESSAGE("python-written example not present; skipping (run the "
		                   "prototype's test_cross_impl.py first for the full bridge)");
		return;
	}
	OutputDirectory out(dir);
	auto const records = out.Scan();
	BOOST_REQUIRE(!records.empty());
	bool saw_run = false, saw_track = false;
	for (auto const& r : records)
	{
		auto const kind = std::string(r.at("kind").as_string());
		if (kind == "run") saw_run = true;
		if (kind == "track") saw_track = true;
	}
	BOOST_CHECK(saw_run);
	BOOST_CHECK(saw_track);
}
