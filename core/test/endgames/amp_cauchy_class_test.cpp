...
BOOST_AUTO_TEST_CASE(cauchy_mp_for_cauchy_class_multiple_variables)
{
	...
	*/
	...
	BOOST_CHECK((my_endgame.FinalApproximation<mpfr>() - correct).norm() < my_endgame.Tolerances().track_tolerance_during_endgame);
...
BOOST_AUTO_TEST_CASE(total_degree_start_system_cauchy_class_used_with_AMP)
{
	...
	*/
	...
	for (auto s : homogenized_solutions)
	{
		...
		if(endgame_success == SuccessCode::Success)
		{
			if((tracker.GetSystem().DehomogenizePoint(my_endgame.FinalApproximation<mpfr>())-correct).norm() < my_endgame.Tolerances().track_tolerance_during_endgame)
			{
