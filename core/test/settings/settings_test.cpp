...
BOOST_AUTO_TEST_CASE(amp_config_refuses_a_system_with_no_degree)
{
    ...
    // accepted recipe for an analytic system.  Deriving one anyway used to produce Phi = 2B from
    // (-1)(-2)B and a NEGATIVE Psi = -B, after which tracking failed obscurely, at 290 bits of
    // precision, with FailedToSelectPrecisionAndStepsize.
    ...
    BOOST_CHECK_NO_THROW(c.SetPhiPsiFromBounds());
    BOOST_CHECK_GT(c.Psi, 0);
    BOOST_CHECK_GT(c.Phi, 0);
